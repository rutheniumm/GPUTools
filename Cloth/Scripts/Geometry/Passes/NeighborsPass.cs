...
// Type: GPUTools.Cloth.Scripts.Geometry.Passes.NeighborsPass
...
namespace GPUTools.Cloth.Scripts.Geometry.Passes
{
  public class NeighborsPass : ICacheCommand
  {
    ...
    public void Cache()
    {
      if (this.data == null || !((UnityEngine.Object) this.mesh != (UnityEngine.Object) null))
        return;
      List<int>[] allTrianglesList = this.CreateAllTrianglesList(this.data.AllTringles, this.data.MeshToPhysicsVerticesMap);
      this.FillStrandedVertices(allTrianglesList);
    ...
    {
      List<int>[] physList = new List<int>[this.data.Particles.Length];
      for (int index = 0; index < triangles.Length; index += 3)
      {
        ...
        this.Add(physList, toPhysicsVertices1, toPhysicsVertices2, toPhysicsVertices3);
        this.Add(physList, toPhysicsVertices2, toPhysicsVertices3, toPhysicsVertices1);
        this.Add(physList, toPhysicsVertices3, toPhysicsVertices1, toPhysicsVertices2);
      }
      return physList;
    }
    ...
    {
      for (int index = 0; index < physList.Length; ++index)
      {
        if (physList[index] == null)
          physList[index] = new List<int>();
      }
    }
    ...
    private void SortList(List<int>[] list)
    {
      ...
      for (int index = 0; index < list.Length; ++index)
      {
        ...
        Vector3 normal = normals[this.data.PhysicsToMeshVerticesMap[index]];
        Vector3 vertex = this.data.Particles[index];
